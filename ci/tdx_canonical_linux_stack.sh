...
long=setuptdx,verifytdx,createtd,runtdqemu,runtdlibvirt,rundirectboot,smoke,pycloudstack_automatedtests,canonical_automatedtests
...
QCOW2_IMG="$GUEST_IMG_DIR/tdx-guest-ubuntu-${UBUNTU_VERSION}-generic.qcow2"
LIBVIRT_CONF=/etc/libvirt/qemu.conf
TDX_TOOLS_DIR="$CUR_DIR/tdx-tools"
DIRECT_BOOT_DIR="$GUEST_TOOLS_DIR/direct-boot"
GUEST_IMAGE="tdx-guest-ubuntu-${UBUNTU_VERSION}-generic.qcow2"
...
source_configs() {
    ...
    source "$CUR_DIR/utils/ubuntu/verify_direct_boot.sh"
}
...
execute_task() {
    while true; do
        case "$1" in
            ...
            --rundirectboot ) log "rundirectboot got selected"; createtd default; rundirectboot; shift ;;
